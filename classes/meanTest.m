classdef meanTest < generalTest1D
    %MEANTEST implements the shift-in-mean test based on the abstract class generalTest1D
    %   The constructor and all relevant distribution related functions are implemented.
    %   This class is used to generate the results presented in section 7.1 of 
    %
    %       D. Reinhard, M. Fauss, and A. M. Zoubir, “Bayesian Sequential Joint Detection and Estimation,” Sequential Analysis, 2018. (submitted).
    %
    %   Signal model:
    %
    %       H_0:    p(x | theta) ~ N(mu_0, sigma^2)  mu_0 ~ p(mu_0 | H_0)
    %       H_1:    p(x | theta) ~ N(mu_1, sigma^2)  mu_1 ~ p(mu_1 | H_1)
    %
    %   meanTest Properties:
    %       sigmaSq     -   variance of the conditional distribution p(x | theta)
    %       llh         -   logarithmic version of the likelihood; normalization factor missing
    %       lh          -   likelihood; normalization factor missing
    %
    %   meanTest Methods:
    %       meanTest         -   constructor
...
% ----------------------------------------------
    properties (SetAccess = private)
        sigmaSq;    % variance of the normal likelihood
    end
...
% ----------------------------------------------
    properties(Constant)
        llh = @(xb,muVec,sSq,n) -(xb - muVec).^2/(2*sSq/n); % logarithmic version of the conditional distribution p( x | mu) ; IMPORTANT: normalization constant is missing
        lh = @(xb,muVec,sSq,n) exp(-(xb - muVec).^2/(2*sSq/n)); %conditional distribution p( x | mu) ; IMPORTANT: normalization constant is missing
    end
...
% ----------------------------------------------    
    methods
        ...
        function obj=meanTest(Ntest, thetaGrid, priorGrid, minStat, maxStat, Nstat,x, piVec, constr,reg,priorGen, sigmaSq,varargin)
        %MEANTEST constructs an object of the class meanTest
        ...
        %       sigmaSq     variance of the conditional distribution
        %       varargin
        ...
        %       obj         the constructed object
            ...
            else % use the passed one otherwise
            ...
            obj.Nstat = Nstat;      % store number of grid points for statistic
            obj.x = x(:);           % reshape and store grid of observation space
            obj.dx = x(2)-x(1);     % aux variable: space of this grid
            % check whether we have four constraints...
            if numel(constr) ~= 4
            ...
            % check range of regularization parameter ...
            if reg < 0
               error('regularization parameter cannot be negative!'); 
            elseif reg > 1
            ...
            % check whether prior vector for hypotheses has exactly two elements
            if numel(piVec) ~= 2
               error('piVec must have exactly two elements');
            end
            obj.piVec = piVec(:);   % store and reshape prior probabilities of hypotheses
        ...
        %GENERATEDATA generate Ntest samples for a given mean theta
            data=randn(obj.Ntest,1)*sqrt(obj.sigmaSq) + theta;
        end
        ...
        %
            ...
            postLog = bsxfun(@plus, obj.llh(stat,obj.thetaGrid,obj.sigmaSq,n), lPrior); % unnormalized version of the posterior
            normConst = logOfSum_array(postLog,2);    % getting normalization constant
            postLog = bsxfun(@minus,postLog,normConst) - log(obj.dt);   % normalizing
        end
        ...
        %   
            ...
            post = (obj.lh(stat,obj.thetaGrid,obj.sigmaSq,n).*prior);   % getting unnormalized posterior
            normConst = sum(post,2);    % getting normalization constant
            post = post/(normConst*obj.dt);     % normalizing
        end
        ...
        %       postPred    posterior predictive
            post = exp(obj.calcLogPosterior(n));
            
            % getting unnormalized likelihood
            lh = exp(obj.llh(obj.x,obj.thetaGrid,obj.sigmaSq,1)).'; %#ok<PROPLC>
            % calculating unnormalized posterior predictive
            postPred = post*lh*obj.dt; %#ok<PROPLC>
            % normalizing posterior predictive
    ...
    methods (Static)
        ...
        %time n to n+1 given statistic stat and new observation x
            statNew = bsxfun(@plus, (n-1)*stat/n, x/n);
        end
    end
